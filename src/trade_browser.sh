...
#
# Usage: ./trade_browser.sh [config_file] [output_file]
#
# If no arguments are provided, uses default configuration

# Configuration
SCRIPT_DIR=$(dirname "$(realpath "$0")") || true
DEFAULT_CONFIG="${SCRIPT_DIR}/trade_browser_auto.json"
...
if ! node "${SCRIPT_DIR}/trade_browser_cli.js" "${CONFIG_FILE}" "${OUTPUT_FILE}"; then
    printf "\n" || true
