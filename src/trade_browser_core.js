...
HMB.config = {
  title: 'USDA ERS Vegetable Trade',
  chartSettings: {
    width: '100%',
    height: '100%',
...
HMB.state = {
  ...
  chartMode: 'regular', // 'regular' or 'annual'
...
HMB.initTitleInput = function() {
  const reportTitleInput = document.getElementById('report-title');
  if (reportTitleInput) {
    console.log('Setting up title input listener');
    // Use 'change' event to only update when user leaves the field or presses enter
    reportTitleInput.addEventListener('change', (e) => {
      const newTitle = e.target.value || 'USDA ERS Vegetable Trade';
      HMB.config.title = newTitle;
      console.log('Title changed to:', HMB.config.title);
      // Redraw chart with new title if we have data
      if (HMB.state.selectedMetrics.length > 0) {
        HMB.renderChart();
      }
    });
  } else {
    console.warn('Report title input not found');
  }
};
...
HMB.initDatePickers = function() {
  ...
  const dataStartDate = new Date(2017, 0, 1); // January 1, 2017
  const dataEndDate = new Date(2025, 9, 31);  // October 31, 2025 (latest data available)
