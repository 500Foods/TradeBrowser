/**
 * Trade Browser - Data Loading Functions
 * Data loading, caching, and CSV processing
 *
 * @version 1.0.0
 * @license MIT
 */
...
HMB.loadCSVData = function() {
  const csvFile = 'veg-trade-data.csv';
  ...
  fetch(csvFile)
    ...
    .then(csvText => {
      ...
      this.state.tradeData = data;
      
      // Extract available metrics (columns)
    ...
    .catch(error => {
      ...
      this.showError('Failed to load trade data. Please check the CSV file.');
      this.state.isLoading = false;
    });
};

// Parse CSV text into structured data
HMB.parseCSV = function(csvText) {
...
HMB.parseCSVLine = function(line) {
  const result = [];
  let current = '';
  let inQuotes = false;
...
// Handle date range changes - simply filter and re-render
HMB.handleDateRangeChange = function() {
  ...
  // Filter the data by the new date range and re-render the chart
